...
export async function getDynamicOptimismRollupSdk(l1Signer: Signer, l2Signer: Signer): Promise<OptimismRollupSdk> {
  ...
  return {
    l1StandardBridge: new Contract(addresses.l1.standardBridge, new Interface([]), l1Signer),
    l1XDomainMessenger: new Contract(
      addresses.l1.xDomainMessenger,
      getContractDefinition('L1CrossDomainMessenger').abi,
      l1Signer,
    ),
    l1StateCommitmentChain: new Contract(addresses.l1.stateCommitmentChain, new Interface([]), l1Signer),
    l2StandardBridge: new Contract(addresses.l2.standardBridge, new Interface([]), l2Signer),
